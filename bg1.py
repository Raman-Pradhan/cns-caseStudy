...
# Helper functions for RSA encryption and decryption (same as original code)
# ...
        ...
        if data_type == 'number':
            ...
            if uploaded_file is not None:
                ...
                if st.button("Encrypt"):
                    ...
                    st.success(f"Encrypted file content saved to {encrypted_file_path}")
                    st.info(f"Public Key (n, e): ({n}, {e}), Private Key (d): {d}")
            ...
            if uploaded_image is not None:
                ...
                if st.button("Encrypt"):
                    ...
                    st.success(f"Image encrypted and saved to: {encrypted_path}")
                    st.info(f"Public Key (n, e): ({n}, {e}), Private Key (d): {d}")
                    st.session_state.encrypted_img_data = encrypted_img_data

    # Decryption part in the Streamlit app
    elif mode == 'decrypt':
        

    # Layout to display 'd' and 'n' side by side
        col1, col2 = st.columns(2)
        with col1:
            ...
            if encrypted_file is not None:
                ...
                if st.button("Decrypt"):
                    try:
                        ...
                        st.success(f"Decrypted file content saved to {decrypted_file_path}")
                    except ValueError:
            ...
            if uploaded_encrypted_image is not None:
                ...
                if st.button("Decrypt"):
                    if 'encrypted_img_data' in st.session_state:
                        ...
                        st.success(f"Image decrypted and saved to: {decrypted_path}")
                    else:
